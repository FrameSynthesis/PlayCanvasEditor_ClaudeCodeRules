...
VSCode PlayCanvas 拡張によるローカル同期環境を前提としています。
...
Claude Code がローカルファイルを編集した後、VSCode 上で手動で Save（Ctrl+S）を行ってください。Save をトリガーに PlayCanvas 拡張がブラウザ側のエディタへ変更を同期し、確実に反映されます。

- 新規ファイルの作成は Editor API 経由で自動的に行われます（ローカルに同期後、編集可能になります）
- スクリプトの attribute パラメータを変更した場合は `/parse-scripts` コマンドでパースを実行してください
